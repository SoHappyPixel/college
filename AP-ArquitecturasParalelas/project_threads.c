    ...
    for(areaX=190; areaX < 260; areaX+=1){
        for(areaY=700; areaY< 1000; areaY+=1){
            ...
            for(bugX=(areaX+(*iniRow)); bugX < (areaX+(*iniRow))+48; bugX+=NTHREADS){
                ...
                pBug= (uchar *) bug->imageData + auxRow * bug->widthStep; auxRow++;
...
int main(int argc, char** argv){
    ...
    while(frame){
        ...
        for(cont = 0; cont < NTHREADS; cont++) pthread_create(&threadsID[cont], NULL, (void *) &cvThread, (void *) &cont);
        
        for(cont = 0; cont < NTHREADS; cont++) pthread_join(threadsID[cont], NULL);
